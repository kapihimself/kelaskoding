...
/**
 * Workspace component manages the IDE state including code, output, and success status.
 * Optimized with React.useDeferredValue and useCallback to prevent blocking UI during typing.
 */
export default function Workspace({ lesson, nextLessonId, trackId }: WorkspaceProps) {
  const [code, setCode] = useState(lesson.initialCode);
  const deferredCode = useDeferredValue(code); // Throttles expensive preview/validation updates
  ...
  const handleQuizSuccess = useCallback(() => {
    ...
    if (lesson.id === 'se-1' || lesson.id === 'ai-1') addBadge('Coder Pemula');
    setShowQuiz(false);
  }, [lesson.id, lesson.xpGain, completeLesson, addBadge]);
