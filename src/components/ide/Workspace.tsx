...
/**
 * Workspace is optimized to maintain editor responsiveness.
 * It uses useDeferredValue for the 'code' prop passed to PreviewPanel
 * to ensure that expensive preview renders don't block user input.
 * All event handlers are wrapped in useCallback for stable child references.
 */
export default function Workspace({ lesson, nextLessonId, trackId }: WorkspaceProps) {
  ...
  // useDeferredValue ensures typing remains smooth even if preview re-renders are heavy
  ...
  }, [code, lesson, completeLesson, addBadge]);
  ...
  const handleQuizSuccess = useCallback(() => {
    ...
    if (lesson.id === 'se-1' || lesson.id === 'ai-1') addBadge('Coder Pemula');
    setShowQuiz(false);
  }, [lesson, completeLesson, addBadge]);
