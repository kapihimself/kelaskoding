...
export default function Workspace({ lesson, nextLessonId, trackId }: WorkspaceProps) {
  ...
  const completeLesson = useStore(useCallback((state) => state.completeLesson, []));
  const addBadge = useStore(useCallback((state) => state.addBadge, []));
  ...
  }, [code, lesson.id, lesson.validationRules, lesson.previewMode, lesson.xpGain, lesson.quiz, completeLesson, addBadge]);
