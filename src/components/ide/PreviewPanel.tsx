...
export default function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  /** PERFORMANCE:
   * Using useMemo here instead of useEffect + useState ensures the document
   * is calculated during the render phase. Since this component receives
   * 'deferredCode', React handles the scheduling, making the manual
   * setTimeout debouncing redundant.
   */
  const doc = useMemo(() => {
    if (mode !== 'html') return '';
