...
/** PERFORMANCE: Wrapped in memo to prevent re-renders when parent state changes but deferredCode hasn't updated yet */
const PreviewPanel = memo(function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  /** PERFORMANCE: Use useMemo to derive doc directly from props to avoid additional re-render from useState + useEffect */
  const doc = useMemo(() => {
    if (mode !== 'html') return '';
