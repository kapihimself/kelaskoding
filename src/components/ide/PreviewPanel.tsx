...
/** PERFORMANCE: Memoize and use deferred updates to prevent typing lag */
const PreviewPanel = memo(function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  /** PERFORMANCE: Use useMemo for derived state to avoid extra render cycles and useEffect synchronization */
  const doc = useMemo(() => {
    if (mode !== 'html') return '';
    return `
      <html>
