...
/**
 * PreviewPanel renders the HTML preview or terminal output.
 * Wrapped in React.memo and uses useDeferredValue (passed via props)
 * to ensure smooth typing performance.
 */
const PreviewPanel = memo(function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  ...
  useEffect(() => {
    if (mode === 'html') {
      // Internal debouncing removed in favor of useDeferredValue in Workspace.tsx
      setDoc(`
        <html>
          <head>
            <style>
