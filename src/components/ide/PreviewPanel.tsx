...
/**
 * PreviewPanel component renders the live preview or terminal output.
 * Wrapped in React.memo and uses useDeferredValue from parent to optimize updates.
 */
const PreviewPanel = memo(function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  ...
  useEffect(() => {
    if (mode === 'html') {
      // Internal debouncing removed in favor of useDeferredValue in parent.
      setDoc(`
        <html>
          <head>
            <style>
