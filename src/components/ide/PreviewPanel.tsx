...
/**
 * Memoized PreviewPanel that uses deferred code updates for smoother typing.
 */
const PreviewPanel = React.memo(function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  ...
  useEffect(() => {
    if (mode === 'html') {
      // Internal debouncing is no longer needed as we use useDeferredValue in the parent.
      setDoc(`
        <html>
          <head>
            <style>
