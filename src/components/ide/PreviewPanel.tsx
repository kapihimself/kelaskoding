...
/**
 * PreviewPanel renders the live HTML preview or terminal output.
 * Optimized with React.memo to decouple from immediate editor state updates.
 */
const PreviewPanel = memo(function PreviewPanel({ code, mode, output }: PreviewPanelProps) {
  ...
  useEffect(() => {
    if (mode === 'html') {
      // Internal timeout removed as we now rely on useDeferredValue from the parent
      // for smoother, non-blocking updates.
      setDoc(`
        <html>
          <head>
            <style>
